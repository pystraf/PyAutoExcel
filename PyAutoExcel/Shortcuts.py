...
def save_pdf(
    ...
    """
    Writing data into a PDF file.

    :param data: 2D list of data to write into the PDF file.
    :type data: list[list]
    :param output_path: The path of the output PDF file.
    :type output_path: str
    :param table_generator: The table generator to use.
    :type table_generator: type
    :param options: A dictionary of options to pass to the PDF writer.
    :type options: dict, optional
    :return: None
    """
    from .Tiny.PdfWriter import PDFKIT_PdfWriter

    writer = PDFKIT_PdfWriter()
    for name, value in options.items():
        writer.pdf_option(opt_name=name, opt=value)
    writer.table("test", data, html_generator=table_generator)
...
def save_image(
    ...
    """
    Writing data into an image file.

    :param data: 2D list of data to write into the image file.
    :type data: list[list]
    :param output_path: The path of the output image file.
    :type output_path: str
    :param table_generator: The table generator to use.
    :type table_generator: type
    :param options: A dictionary of options to pass to the image writer.
    :type options: dict, optional
    """
    from .Tiny.ImageWriter import IMGKIT_ImageWriter

    writer = IMGKIT_ImageWriter()
    for name, value in options.items():
        writer.image_option(opt_name=name, opt=value)
    writer.table("test", data, html_generator=table_generator)
    writer.write(output_path)
